...
int main(){
	...
	int loc,start_loc,object_code,opc,target_address,loc_1,buffer;
	char mnemonics[30],label_1[30],label_2[100];
	char lbl1[25], mne[25], lbl2[25], opcode[25],mne1[25],str[25],num[25],xyz[30],buffer_2[30];
   ...
   while(fscanf(f_inter,"%X %s %s %s",&loc ,label_1,mnemonics,label_2) == 4){
   		if((strcmp(mnemonics,"RESB") != 0) && (strcmp(mnemonics,"RESW") != 0)  && (strcmp(mnemonics,"WORD") != 0)  && strcmp(mnemonics,"BYTE") != 0){
   	 		fscanf(fp5,"%*s %X" , &opc);
   	 		object_code = opc;
   	 		while(fscanf(f_sym,"%X %s",&loc_1,lbl1) == 2){
   	 			...
   	 			if(label_2[length-1] == 'X') {
   	 				...
   	 				if(strcmp(buffer_2,lbl1) == 0){
   	 					int x = 0x0;
   	 					 x = loc_1;
   	 					 while(x >= 16)
   	 						x=x/16;
   	 					if(x < 8)
   	 						target_address = loc_1 + 32768;
   	 				    else
   	 				    	target_address = loc_1;
   	 			    }
   	 		    }
   	 		    else{
   	 		    	if(strcmp(label_2,lbl1) == 0){
   	 		    		 int x = loc_1;
   	 					while(x >= 16)
   	 						x=x/16;
   	 					if(x > 8)
